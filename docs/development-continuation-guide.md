...
## 2. Continuation Rules for Agents

To maintain the integrity of the forge, all agentic contributors must follow these steps:
...
Update the Brain (`G-Codex-brain/`) and `docs/` whenever a slice changes a contract or adds a capability. Never imply that unbuilt features exist.

### D. No Discovery Memory
Make continuation easy for the next agent by recording exact commit hashes and implementation details in `MERGE_LOG.md`.

---

## 3. How To Continue Development

1.  **Verify Baseline:** Run `python3 main_forge.py --name "check" --primitive "cube"` to ensure the environment is healthy.
